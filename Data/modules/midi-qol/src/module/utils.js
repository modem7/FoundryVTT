...
*/
export function findNearby(disposition, token /*Token | undefined */, distance, options = { maxSize: undefined, includeIncapacitated: false }) {
	if (!token)
		return [];
	...
	if (canvas?.grid?.grid instanceof SquareGrid) {
		...
		while (allies.length > 1) {
			...
			for (let ally of allies) {
				...
				if (installedModules.get("dfreds-convenient-effects")) {
					//@ts-ignore
					if (actor?.effects.some(ef => ef.label === game.dfreds.effects._incapacitated.label))
						continue;
				}
				if (checkRule("checkFlanking") === "ceflankedNoconga" && installedModules.get("dfreds-convenient-effects")) {
					//@ts-ignore
